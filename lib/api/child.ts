...
export const handleInviteChild = async (
  tx: Prisma.TransactionClient,
  childId: number,
  inviteParentTelegramId: number
) => {
  ...
  const parent = await tx.parentUser.findUnique({
    where: { telegram_id: String(inviteParentTelegramId) },
  ...
  if (!parent) {
    console.warn(`Родитель с ID ${inviteParentTelegramId} не найден.`);
    return;
  }

  //Создаем связь между родителем и ребенком
  await createParentChildRelation(tx, parent.id, childId);
